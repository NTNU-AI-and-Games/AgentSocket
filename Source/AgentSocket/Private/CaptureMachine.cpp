...
void UCaptureMachine::Close()
{
	...
	// The following is disabled to prevent ue4 crash
	/*
	if (m_MemDC)
	{
		::DeleteDC(m_MemDC);
...
// Convert char* to TArray<FColor>
TArray<FColor> UCaptureMachine::GetBitmapBufferCopy() {
	int area = m_WindowSize.X * m_WindowSize.Y;
	TArray<FColor> copy;
	copy.Init(FColor::Black, area);

	// Loop and flip image
	for (int i = 0; i < area; i++)
	{
		int _y = i / m_WindowSize.X;
		int _x = i % m_WindowSize.X;
		int flipIndex = (m_WindowSize.Y - 1 - _y) * m_WindowSize.X + _x;
		copy[flipIndex].R = m_BitmapBuffer[i * 4 + 2];
		copy[flipIndex].G = m_BitmapBuffer[i * 4 + 1];
		copy[flipIndex].B = m_BitmapBuffer[i * 4 + 0];
		copy[flipIndex].A = m_BitmapBuffer[i * 4 + 3];
	}
	return copy;
}
...
bool UCaptureMachine::DoCapture()
{
	...
	if (Properties.CheckWindowSize)
	{
		...
		{
			ReCreateTexture();
			ChangeTextureSize.Broadcast(TextureTarget);
		}
...
// FIXME: REMOVE THIS, as we don't need the texture2d version of the captured image
...
void UCaptureMachine::ReCreateTexture()
{
	...
	/*FString bitmapString = "";

	for (char c = *m_BitmapBuffer, nPixels = m_WindowSize.X * m_WindowSize.Y * 4; nPixels > 0; c = *(m_BitmapBuffer + nPixels), --nPixels) {
		bitmapString.AppendChar(c);
	}

	UE_LOG(LogTemp, Warning, TEXT("data: %s"), *bitmapString);*/
